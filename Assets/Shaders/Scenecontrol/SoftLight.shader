...
Shader "Custom/BlendModes/SoftLight"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed blendSoftLight(fixed base, fixed blend)
			{
				return  (blend < 0.5) ? 
			...
			fixed3 blendSoftLight(fixed3 base, fixed3 blend, fixed opacity)
			{
			...
			fixed3 frag(v2f i) : SV_Target
			{
				...
				return blendSoftLight(baseColor, texColor, texColor.a);
			}
			ENDCG
		}
	}
