...
namespace ArcCreate.Gameplay.Utility
{
    public static class ArcMeshGenerator
    {
        ...
        public static Mesh GetSegmentMesh(bool isTrace)
        {
            if (isTrace)
            {
                if (cachedTraceMesh == null) cachedTraceMesh = GenerateMesh(Values.TraceMeshOffset);

                return cachedTraceMesh;
            }

            if (cachedArcMesh == null) cachedArcMesh = GenerateMesh(Values.ArcMeshOffset);
        ...
        public static Mesh GetShadowMesh(bool isTrace)
        {
            if (isTrace)
            {
                if (cachedTraceShadowMesh == null) cachedTraceShadowMesh = GenerateShadowMesh(Values.TraceMeshOffset);

                return cachedTraceShadowMesh;
            }

            if (cachedArcShadowMesh == null) cachedArcShadowMesh = GenerateShadowMesh(Values.ArcMeshOffset);
        ...
        public static Mesh GetHeadMesh(bool isTrace)
        {
            if (isTrace)
            {
                if (cachedTraceHeadMesh == null) cachedTraceHeadMesh = GenerateHeadMesh(Values.TraceMeshOffset);

                return cachedTraceHeadMesh;
            }

            if (cachedArcHeadMesh == null) cachedArcHeadMesh = GenerateHeadMesh(Values.ArcMeshOffset);
            ...
            Arc arc, List<Vector3> vertices, List<int> triangles, Vector3 pos, Vector3 scl)
        {
            ...
            var offset = arc.IsTrace ? Values.TraceMeshOffset : Values.ArcMeshOffset;
            var offsetHalf = offset / 2;

            var segmentCount = Mathf.CeilToInt((arc.EndTiming - arc.Timing) / arc.SegmentLength);
            segmentCount = Mathf.Max(segmentCount, 1);
            var segmentLength = arc.SegmentLength;
            var tg = arc.TimingGroupInstance;

            var baseX = ArcFormula.ArcXToWorld(arc.XStart);
            var baseY = ArcFormula.ArcYToWorld(arc.YStart);
            var baseZ = ArcFormula.FloorPositionToZ(arc.FloorPosition);
            ...
            if (arc.IsFirstArcOfGroup)
            {
                ...
                // 2 | 3
                //  \0/
                var arcIsBackward = tg.GetFloorPosition(Mathf.Min(arc.Timing + (int)segmentLength, arc.EndTiming)) <
                                    arc.FloorPosition;
                ...
                {
                    var timing = Mathf.RoundToInt(arc.Timing + segmentLength * (i + 1));
                    timing = Mathf.Min(timing, arc.EndTiming);
                    var x = timing == arc.EndTiming ? ArcFormula.ArcXToWorld(arc.XEnd) : arc.WorldXAt(timing);
                    var y = timing == arc.EndTiming ? ArcFormula.ArcYToWorld(arc.YEnd) : arc.WorldYAt(timing);
                    var z = ArcFormula.FloorPositionToZ(tg.GetFloorPosition(timing));
