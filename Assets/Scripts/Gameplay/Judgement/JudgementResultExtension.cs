namespace ArcCreate.Gameplay.Judgement
{
    public static class JudgementResultExtension
    {
        ...
        {
            return (inputTime - trueTime).CalculateJudgeResult();
        }

        public static JudgementResult CalculateJudgeResult(this int offset, bool isArcTap = false)
        {
            if (offset >= Values.GoodJudgeWindow) return JudgementResult.MissLate;

            if (offset >= Values.PerfectJudgeWindow) return JudgementResult.GoodLate;

            if (offset >= Values.MaxJudgeWindow) return isArcTap ? JudgementResult.Max : JudgementResult.PerfectLate;

            if (offset >= -Values.MaxJudgeWindow) return JudgementResult.Max;

            if (offset >= -Values.PerfectJudgeWindow)
                return isArcTap ? JudgementResult.Max : JudgementResult.PerfectEarly;

            if (offset >= -Values.GoodJudgeWindow) return JudgementResult.GoodEarly;
