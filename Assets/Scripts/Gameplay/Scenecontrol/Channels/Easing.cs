...
namespace ArcCreate.Gameplay.Scenecontrol
{
    ...
    public class Easing
    {
        ...
        private static readonly Dictionary<string, Func<float, float, float, float>> StringMapping = new()
        {
            { "linear", Linear },
            { "l", Linear },
            { "inconstant", InConst },
        ...
        public static float InElastic(float start, float end, float x)
        {
            ...
            return start + (end - start) *
                (x == 0 ? 0 : x == 1 ? 1 : -Pow(2, 10 * x - 10) * Sin((x * 10 - 10.75f) * c4));
        }
        ...
        public static float OutBounce(float start, float end, float x)
        {
            ...
            return start + (end - start) * (x < 1 / d1 ? 1 * x * x :
                x < 2 / d1 ? 1 * (x -= 1.5f / d1) * x + 0.75f :
                x < 2.5 / d1 ? 1 * (x -= 2.25f / d1) * x + 0.9375f : n1 * (x -= 2.625f / d1) * x + 0.984375f);
        }

        public static float InOutBounce(float start, float end, float x)
        {
            return start + (end - start) *
                (x < 0.5 ? (1 - OutBounce(0, 1, 1 - 2 * x)) / 2 : (1 + OutBounce(0, 1, 2 * x - 1)) / 2);
        }

        [MoonSharpHidden]
        public static Func<float, float, float, float> FromString(string s)
        {
            if (string.IsNullOrEmpty(s) || !StringMapping.ContainsKey(s)) return Linear;
