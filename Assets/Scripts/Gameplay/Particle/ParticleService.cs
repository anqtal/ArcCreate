...
namespace ArcCreate.Gameplay.Particle
{
    public class ParticleService : MonoBehaviour
    {
        ...
        [Header("Text particle")] [SerializeField]
        ...
        [Header("Early late display")] [SerializeField]
        private TMP_Text earlyLateText;
        ...
        [Header("Other particles")] [SerializeField]
        ...
        [Header("Parents")] [SerializeField] private Transform tapParticleParent;
        ...
        [Header("Numbers")] [SerializeField] private int tapParticlePoolCount = 50;
        ...
        private readonly List<LongNote> arcParticlesToPrune = new();
        private readonly List<LongNote> holdParticlesToPrune = new();
        private readonly char[] offsetCharArray = new char[8];

        private readonly Dictionary<LongNote, ParticleSchedule> playingArcParticles = new(4);

        private readonly Dictionary<LongNote, ParticleSchedule> playingHoldParticles = new(6);
        ...
        {
            var currentRealTime = Time.time;

            if (currentRealTime <= lastEarlyLateRealTime + earlyLateLength)
            {
                var x = 1 - (currentRealTime - lastEarlyLateRealTime) / earlyLateLength;
                var easeCubicOut = 1 - x * x * x;
                var y = earlyLateFromY + (earlyLateToY - earlyLateFromY) * easeCubicOut;
                var a = 1 - easeCubicOut;

                earlyLateTransform.anchoredPosition = new Vector3(0, y, 0);

                var c = earlyLateText.color;
                c.a = a;
            ...
            for (var i = 0; i < arcParticlesToPrune.Count; i++)
            {
                var reference = arcParticlesToPrune[i];
            ...
            for (var i = 0; i < holdParticlesToPrune.Count; i++)
            {
                var reference = holdParticlesToPrune[i];
        ...
        public void PlayTapParticle(Vector3 worldPosition, JudgementResult result)
        {
            ...
            var screenPos = ConvertToScreen(worldPosition);
            var ps = tapParticlePool.Get();
        ...
        public void PlayTextParticle(Vector3 worldPosition, JudgementResult result, Option<int> offset)
        {
            ...
            if (result.IsEarly() && (result.IsGood() || Settings.ShowEarlyLatePerfect.Value))
            {
                ...
                {
                    SetOffsetNumber(offsetCharArray, offset.Value, out var length);
        ...
        public void PlayHoldParticle(LongNote reference, Vector3 worldPosition)
        {
            ...
            {
                var ps = playingHoldParticles[reference];
        ...
        public void PlayArcParticle(int colorId, LongNote reference, Vector3 worldPosition)
        {
            ...
            {
                var ps = playingArcParticles[reference];
        ...
        public void SetHoldParticleSkin(Color colorMin, Color colorMax, Gradient fromGradient, Gradient toGradient,
            Color colorGrid)
        {
            var pts = holdNoteParticlePrefab.GetComponent<ParticleSystem>();
            var module = pts.main;
            var colorModule = pts.colorOverLifetime;
        ...
        {
            var pos = (EarlyLateTextPosition)val;
            var yAnchor = 0.5f;
            switch (pos)
            {
        ...
        private Vector2 ConvertToScreen(Vector3 world)
        {
            ...
            return new Vector2(viewport.x * screenParticlesRect.rect.width,
                viewport.y * screenParticlesRect.rect.height);
        }
        ...
        private void SetOffsetNumber(char[] charArray, int value, out int length)
        {
            ...
            for (var i = 3; i < charArray.Length; i++) charArray[i - 3] = charArray[i];
