...
namespace ArcCreate.SceneTransition
{
    public class FitToContentText : ReactiveText
    {
        ...
        protected override void OnTextChange(string text)
        {
            ...
            var w = Mathf.Clamp(CachedText.preferredWidth + padding.x, minSize.x, maxSize.x);
            var h = Mathf.Clamp(CachedText.preferredHeight + padding.y, minSize.y, maxSize.y);
            ...
            var shrink = Mathf.Clamp(w / (CachedText.preferredWidth + padding.x), 0.25f, 1);
