...
namespace ArcCreate.Utility.RangeTree
{
    ...
    {
        private UnorderedList<RangeValuePair<T>> items = new(0);
        ...
        /// </summary>
        /// <param name= from">The lower end of the query.
        /// </param>
        /// <param name= to">The upper end of the query.
        /// </param>
        public RangeTreeEnumerator<T> this[double from, double to] => new(root, from, to);
        ...
        {
            if (from > to) throw new ArgumentOutOfRangeException($"{nameof(from)} cannot be larger than {nameof(to)}");
