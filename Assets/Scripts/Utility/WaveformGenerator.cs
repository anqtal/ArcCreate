...
namespace ArcCreate.Utility
{
    public static class WaveformGenerator
    {
        ...
        public static Texture2D EncodeTexture(AudioClip clip)
        {
            ...
            while (maxSize > 1024)
            {
                var samplePixelCount = Mathf.CeilToInt(sampleCount / AverageSampleCount / 4f);
            ...
            if (texture == null)
                throw new Exception("Could not create waveform texture, no suitable texture size found");
            ...
            {
                float s1 = 0;
                for (var i = sample; i < sample + AverageSampleCount; i++)
        ...
        {
            if (clip == null || Mathf.Approximately(clip.length, 0)) return 0;

            var samplePerSecond = clip.samples / clip.length;
